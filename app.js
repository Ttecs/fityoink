...
// ── Host selector ─────────────────────────────────────────────────────────────
let selectedHost = "fuckingfast";

function updateBrdGuide() {
  const needsBrd = selectedHost === "datanodes" && !settings.sbrWs?.trim();
  $("brd-guide").classList.toggle("hidden", !needsBrd);
  $("btn-fetch").disabled = needsBrd;
}

document.querySelectorAll(".host-pill").forEach(pill => {
  pill.onclick = () => {
    selectedHost = pill.dataset.host;
    document.querySelectorAll(".host-pill").forEach(p => p.classList.toggle("active", p === pill));
    updateBrdGuide();
  };
});

$("brd-guide-settings").onclick = () => openSettings(true);

// ── Fetch links ───────────────────────────────────────────────────────────────
$("btn-fetch").onclick = fetchLinks;
$("game-url").addEventListener("keydown", e => { if (e.key === "Enter") fetchLinks(); });

async function fetchLinks() {
  const url = $("game-url").value.trim();
  if (!url) return;
  $("btn-fetch").disabled = true;
  setStatus("fetch-status", "⏳ Fetching links...", "");
  try {
    links = await window.api.fetchLinks(url, selectedHost);
    if (!links.length) { setStatus("fetch-status", "No links found.", "error"); return; }
    renderFileList();
